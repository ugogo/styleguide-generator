 ...
 * Replace contentStr options with generated content
  * Generate index.{layoutExtension} */

Styleguide.prototype.onePageGenerateIndex = function () {
  var ext = this.options.layout;
  var str = u.readFile( this.options.layout );
  var content = this.options.onePage.content;
  var indexPath;

  ext = ext.split('.').pop();
  content = content.join('');
  str = str.replace( this.options.onePage.contentStr, content );

  indexPath = this.options.distFolder +  'index.' + ext;

  u.createFile( indexPath, str );
};
...
Styleguide.prototype.generate = function (cb) {
  ...
  files.forEach( function( path ){
    ...
    if (onePage) {
      self.options.onePage.content.push(htmlContent);
    }
    else {
