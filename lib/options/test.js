...
module.exports = function () {
	...
	if (!validType) {
		return errors.log('Type is not valid' + '\n' + '> Accepted types: onepage, one-page, components', 'opts.type');
	}
	...
	if (opts.type === 'onepage' || opts.type === 'one-page') {
		...
		if (!Utils.exist(opts.layout)) {
			return errors.log('Layout not found', 'opts.layout');
		}

		var layoutContent = Utils.read.file(opts.layout, 'utf8');
	...
	this.opts.onePage = opts.type === 'onepage' || opts.type === 'one-page';
