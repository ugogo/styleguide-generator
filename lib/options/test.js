...
module.exports = function () {
	...
	var beforeCompilationClone = opts.beforeCompilation.bind({});
	var afterCompilationClone = opts.afterCompilation.bind({});
	...
	if (typeof beforeCompilationClone !== 'function' ) {
		return errors.log('beforeCompilation isn\'t a function');
	}

	if (typeof beforeCompilationClone('# test content', 'path/to/fake') !== 'string' ) {
		return errors.log('beforeCompilation should return a string');
	}

	if (typeof afterCompilationClone !== 'function' ) {
		return errors.log('afterCompilation isn\'t a function');
	}

	if (typeof afterCompilationClone('# test content', 'path/to/test') !== 'string' ) {
